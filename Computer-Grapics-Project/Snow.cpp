                            ...
                            std::vector<glm::vec2>& texCoords) const
{
    ...
    // 높이에 따른 텍스처 매핑 계산
    // height가 0.5일 때 옆면은 절반만, 1.0일 때 전체 텍스처 사용
    float heightRatio = height; // 0.5 또는 1.0
    float sideTexTop = heightRatio; // 옆면 상단 텍스처 좌표 (0.5 또는 1.0)
...
bool Snow::canSnowBeGenerated(int gridX, int gridZ) const
{
    ...
    if (getSnowHeightAt(gridX, gridZ) >= 1.0f) {
    ...
    // 2. 기존 눈에 닿으면서, 그 눈이 최대 높이(1.0)인지 확인
